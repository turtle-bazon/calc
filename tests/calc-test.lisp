...
(test eval-hyperbolic-trig
  ...
  (let ((vars (make-hash-table :test #'equal))
    ...
    (is (< (abs (- (calc:eval-rpn "[ 2 4 4 4 5 5 7 9 ] stddev" vars funcs) 2.0)) 0.01))))(test eval-rand (let ((vars (make-hash-table :test (quote equal))) (funcs (make-hash-table :test (quote equal)))) (is (numberp (calc:eval-rpn "1 RAND" vars funcs))) (is (integerp (calc:eval-rpn "10 RANDINT" vars funcs))) (is (< 0 (calc:eval-rpn "10 RANDINT" vars funcs) 10))))