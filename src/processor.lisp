...
(defun process-expression (tr vars funcs)
  (when (> (length tr) 0)
    (handler-case
        (cond
          ((char= (char tr 0) #\#) nil)
          ((handle-assignment tr vars funcs))
          ;; defmacro
          ((handle-defmacro-form tr vars funcs))
          ;; defun
          ((handle-defun-form tr vars funcs))
          ;; Function call
...
(defun handle-assignment (tr vars funcs)
  (when (let ((eq (position (char "=" 0) tr :from-end t))
...
(defun handle-defmacro-form (tr vars funcs)
  (when (let ((pos (search "defmacro " tr :test #'string-equal)))
             (and pos (= pos 0)))
...
(defun handle-defun-form (tr vars funcs)
  (when (let ((pos (search "defun " tr :test #'string-equal)))
             (and pos (= pos 0)))
    (let* ((parts (uiop:split-string tr :separator '(#\Space #\Tab)))
                  (raw-name (second parts))
...
(defun handle-call-form (tr vars funcs)
