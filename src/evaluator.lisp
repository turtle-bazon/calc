...
(defun handle-array (tok stack)
  ...
  (let ((u (string-upcase tok)))
    (cond
      ...
      ((string= u "AMIN")
       (when (< (length stack) 1)
         (error 'calc-error :message "AMIN requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (and (listp arr) arr)
           (error 'calc-error :message "AMIN requires a non-empty array"))
         (cons (reduce #'min arr) stack)))
      ((string= u "AMAX")
       (when (< (length stack) 1)
         (error 'calc-error :message "AMAX requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (and (listp arr) arr)
           (error 'calc-error :message "AMAX requires a non-empty array"))
         (cons (reduce #'max arr) stack)))
      ((string= u "SORT")
       (when (< (length stack) 1)
         (error 'calc-error :message "SORT requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (listp arr)
           (error 'calc-error :message "SORT requires an array"))
         (cons (sort (copy-list arr) #'<) stack)))
      ((string= u "REVERSE")
       (when (< (length stack) 1)
...
(defun handle-stats (tok stack)
  (let ((u (string-upcase tok)))
    (cond
      ((string= u "MEAN")
       (when (< (length stack) 1)
         (error 'calc-error :message "MEAN requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (listp arr)
           (error 'calc-error :message "MEAN requires an array"))
         (when (null arr)
           (error 'calc-error :message "MEAN requires a non-empty array"))
         (cons (/ (apply #'+ arr) (length arr)) stack)))
      ((string= u "MEDIAN")
       (when (< (length stack) 1)
         (error 'calc-error :message "MEDIAN requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (listp arr)
           (error 'calc-error :message "MEDIAN requires an array"))
         (when (null arr)
      ...
      ((string= u "STDDEV")
       (when (< (length stack) 1)
         (error 'calc-error :message "STDDEV requires an array on the stack"))
       (let ((arr (pop stack)))
         (unless (listp arr)
           (error 'calc-error :message "STDDEV requires an array"))
         (when (< (length arr) 2)
