  ...
  (let ((u (string-upcase tok)))
    (cond
      ...
      ((string= u "AMIN")
       ...
       (let ((arr (pop-nonempty-array stack "AMIN requires a non-empty array" "AMIN requires a non-empty array")))
         (cons (reduce (cond
      ...
      ((string= u "AMAX")
       ...
       (let ((arr (pop-nonempty-array stack "AMAX requires a non-empty array" "AMAX requires a non-empty array")))
         (cons (reduce (cond
    ...
    (cond
      ((string= u "MEAN")
       ...
       (let ((arr (pop-array stack "MEAN requires an array")))
         (check-nonempty arr "MEAN requires a non-empty array")
      ...
      ((string= u "MEDIAN")
       ...
       (let ((arr (pop-array stack "MEDIAN requires an array")))
         (check-nonempty arr "MEDIAN requires a non-empty array")
      ...
      ((string= u "STDDEV")
       ...
       (let ((arr (pop-array stack "STDDEV requires an array")))
         (check-min-length arr 2 "STDDEV requires at least two elements")
      ...
      ((string= u "VARIANCE")
       ...
       (let ((arr (pop-array stack "VARIANCE requires an array")))
         (check-min-length arr 2 "VARIANCE requires at least two elements")
         (let* ((mean (mean-of arr))
                (variance (/ (apply #'+ (mapcar (lambda (x) (expt (- x mean) 2)) arr))
                             (length arr))))
           (cons variance stack))))
      ((string= u "RANGE")
       ...
       (let ((arr (pop-nonempty-array stack "RANGE requires a non-empty array" "RANGE requires a non-empty array")))
         (cons (- (reduce #'max arr) (reduce #'min arr)) stack)))
      ((string= u "MODE")
       (check-arity stack 1 "MODE requires an array on the stack")
       (let ((arr (pop-nonempty-array stack "MODE requires a non-empty array" "MODE requires a non-empty array")))
         (let ((sorted (sort (copy-list arr) #'<)))
...
(defun check-index (arr idx message)
  ...
  (when (or (< idx 0) (>= idx (length arr)))
    (error 'calc-error :message message)))
(defun pop-nonempty-array (stack array-msg nonempty-msg)
  "Pop STACK expecting a non-empty array; signal calc-error otherwise."
  (let ((arr (pop-array stack array-msg)))
    (when (null arr)
      (error 'calc-error :message nonempty-msg))
    arr))
(defun check-nonempty (arr message)
  "Signal calc-error if ARR is empty."
  (when (null arr)
    (error 'calc-error :message message)))

(defun check-min-length (arr n message)
  "Signal calc-error if ARR has fewer than N elements."
  (when (< (length arr) n)
    (error 'calc-error :message message)))
